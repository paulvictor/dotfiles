...
in {
  ...
  # Minimal host config: sops secret + microvm user access (no WG)
  host.secrets = {
  ...
  # Full host config including WG (re-enable when needed)
  ...
  # Minimal VM config: password share + openconnect passwordFile (no WG)
  vm.secrets = {config, lib, pkgs, ...}: {
    microvm.shares = [{
      tag = "wordpass";
  ...
  # Full VM config including WG (re-enable when needed)
